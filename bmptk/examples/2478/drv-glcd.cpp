...
Int32U testtbl[8] = {0x00000000, 0x00000001, 0x00000101,
...
Int32U Y_size;
  ...
  pSrc = (bmp_24bitpixel_t *)((Int32U)pBmp + pBmp->pixarray_offset);
  pSrc = (bmp_24bitpixel_t *)((Int32U)pSrc +(pBmp->dip_header.bmp_height-1)*PicLineSize);
  
  X_size = pPic->H_Size;
  Y_size = pPic->V_Size;
  

  for(Int32U i = 0; i < Y_size; i++)
  { 
    for(Int32U j = 0; j < X_size; j++,pData++,pSrc++)
    {
      pData->red = pSrc->red;
    ...
    pSrc -= X_size;
    pSrc = (bmp_24bitpixel_t *)((Int32U)pSrc - PicLineSize);
  }
...
/*************************************************************************
 * Function Name: GLCD_LoadPic
 * Parameters: Int32U X_Left, Int32U Y_Up,  pPic_t pPic, Int32U Mask
 *
 * Return: none
 *
 * Description: Load picture in VRAM memory area
...
{
//Int32U * pData = (Int32U *)LCD_VRAM_BASE_ADDR;
Int32U * pData;
Int32U * pSrc = (Int32U *)(pPic->pPicStream);                           
Int32U X_size;
Int32U Y_size;

  LCD_UPBASE = (Int32U) LCD_Buffer[BuffIndex];
  BuffIndex = (BuffIndex + 1) & 1;
  pData = LCD_Buffer[BuffIndex];
  ...
  LCD_UPBASE = (Int32U) LCD_Buffer[BuffIndex];
  Int32U i, Size = C_GLCD_H_SIZE * C_GLCD_V_SIZE;
  for (i=0; i<Size; i++)
...
void GLCD_ShowPic_in_Box (pPic_t pPic, Point UL, Point DR, Int32U Mask, int ClearFlag)
{
  ...
  Int32U * pData;
  Int32U * pSrc = (Int32U *)(pPic->pPicStream);                           
  int PicSizeX, PicSizeY, i, j;
  
  PicSizeX = pPic->H_Size;
  PicSizeY = pPic->V_Size;

  LCD_UPBASE = (Int32U) LCD_Buffer[ACTIVE_BUFFER];
  ACTIVE_BUFFER = INACTIVE_BUFFER;
//  BuffIndex = (BuffIndex + 1) & 1;
  pData = LCD_Buffer[ACTIVE_BUFFER];
  ...
  // actual refreshing of the buffer
  pData = LCD_Buffer[ACTIVE_BUFFER];
  
  for(i = UL.Y; i <= DR.Y; i++)
  {
    if (i<0)                  // check if the pixels is outside the screen
      continue;
    if (i>=C_GLCD_V_SIZE)     // check if the pixel is outside the screen
      break;
    if (i - UL.Y >= PicSizeY) // check if the pixel is outside the picture
      break;
    for(j = UL.X; j <= DR.X; j++)
    {
      if (j<0)                  // check if the pixels is outside the screen
        continue;
      if (j>=C_GLCD_H_SIZE)     // check if the pixel is outside the screen
        break;
      if (j - UL.X >= PicSizeX) // check if the pixel is outside the picture
        break;
    #if 1   // wtf?! this is working
      static Int32U *pointer;
      pointer = pData + i*FrameSizeX + j;
      *pointer = *(pSrc+(i-UL.Y)*PicSizeX+(j-UL.X))^Mask;
    #else   // but not this...
      *(pData + i*FrameSizeX + j) = *(pSrc+(i-UL.Y)*PicSizeX+(j-UL.X))^Mask;
    #endif
    }
  }
  
  // the second buffer is used to avoid the blinking when the picture is refreshed
  LCD_UPBASE = (Int32U) LCD_Buffer[ACTIVE_BUFFER];
  Int32U Size = C_GLCD_H_SIZE * C_GLCD_V_SIZE;
  for (i=0; i<Size; i++)
    LCD_Buffer[0][i] = LCD_Buffer[1][i];
...
{
  LCD_UPBASE = (Int32U) LCD_Buffer[INACTIVE_BUFFER];
  return;
}
...
void Synchronize_Buffers (void)
{
  ...
  for(i = 0; i < C_GLCD_H_SIZE*C_GLCD_V_SIZE; i++)
      LCD_Buffer [INACTIVE_BUFFER][i] = LCD_Buffer [ACTIVE_BUFFER][i]; 
}
